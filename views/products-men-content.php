...
if(isset($_GET['str'])){
	$strana=$_GET['str'];
	$pocetak=($strana-1)*3;
	$upit=$connection->query("SELECT * FROM proizvod WHERE pol='M' LIMIT $pocetak,$po_strani");
	$proizvod=$upit->fetchAll();
}
else{
	$upit=$connection->query("SELECT * FROM proizvod WHERE pol='M' LIMIT 0,$po_strani");
	$proizvod=$upit->fetchAll();
}
							...
							<div class="product-image-wrapper">
								<div class="single-products">
									<div class="productinfo text-center">
										...
										<h2><?= $product->cena?>$</h2>
										<p><?= $product->ime?></p>
										<a href="#" class="btn btn-default add-to-cart <?= $product->proizvod_id?>" data-id="<?= $product->proizvod_id?>" ><i class="fa fa-shopping-cart"></i>Add to cart</a>
